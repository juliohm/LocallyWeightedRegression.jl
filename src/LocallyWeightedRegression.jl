...
* `weightfunc` - A weight function defined in MLKernels.jl (default to ExponentialKernel())
...
@estimsolver LocalWeightRegress begin
  ...
  @param weightfunc = ExponentialKernel()
...
function solve(problem::EstimationProblem, solver::LocalWeightRegress)
  ...
  for (var,V) in variables(problem)
    ...
    if ndata > 0
      ...
      # determine weight (or kernel) function
      wfun = varparams.weightfunc
