...
@testset "LocallyWeightedRegression.jl" begin
  ...
  @testset "2D regression" begin
    sdata   = PointSetData(OrderedDict(:y => [1.,0.,1.,0.]), [25. 50. 75. 75.;  25. 75. 50. 25.])
    ...
    if visualtests
      @plottest begin
        contourf(sol₃)
        plot!(sdata)
      end joinpath(datadir,"neighbors3.png") !istravis
      @plottest begin
        contourf(sol₄)
        plot!(sdata)
      end joinpath(datadir,"neighbors4.png") !istravis
    end
  end

  @testset "2D haversine" begin
    ...
    sdata   = PointSetData(OrderedDict(:z=>z), collect(zip(x,y)))
    sdomain = begin
        dims = (180, 91)
    ...
    if visualtests
      gr(size=(900,250))
      @plottest contourf(solution) joinpath(datadir,"haversine2D.png") !istravis
    end
  end
end
